...
serve(async (req) => {
  ...
  try {
    ...
    // Authenticate the caller — only the order's owner may initiate its payment
    ...
    // IDOR guard: the caller must own this order
    if (order.customer_id !== user.id) {
      console.warn(`User ${user.id} attempted to pay for order ${order_id} owned by ${order.customer_id}`);
      return new Response(
    ...
    // Do not restart an already-paid order
